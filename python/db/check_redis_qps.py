...
def InquireRedis(ip, port, answer):
    reqproccess = popen("redis-cli -h %s -p %s -a %s info | grep total_commands_processed | awk -F ':' '{print $2}'" % (
    ip, port, answer)).read()
...
def init():
    parser = argparse.ArgumentParser(
        description="example:[ python CheckRedisQps.py -i 10.10.10.10 -p 6379 -a 1234567 --interval 10.0  -c 10000 ]")
